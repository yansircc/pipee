...
export const PI_WEB_PATH_MIME = "application/x-pi-web-path"
export const PI_WEB_DIRECTORY_MIME = "application/x-pi-web-directory"
...
function decodeUriComponent(value: string): string {
  ...
  const decoded = new URL(`https://pi-web.invalid/?value=${escaped}`).searchParams.get("value")
...
function readInternalPath(dataTransfer: DropTransferView): DroppedPath | null {
  const internal = dataTransfer.getData(PI_WEB_PATH_MIME)
...
export function inspectDrop(dataTransfer: DropTransferView): { hasFiles: boolean; hasDirectory: boolean } {
  ...
  const hasInternalPath = types.includes(PI_WEB_PATH_MIME)
  const hasDirectory =
    types.includes(PI_WEB_DIRECTORY_MIME) ||
...
export function writePathDrag(dataTransfer: DataTransfer, path: string, isDirectory: boolean): void {
  ...
  dataTransfer.setData(PI_WEB_PATH_MIME, JSON.stringify({ path, isDirectory } satisfies DroppedPath))
  if (isDirectory) dataTransfer.setData(PI_WEB_DIRECTORY_MIME, "1")
