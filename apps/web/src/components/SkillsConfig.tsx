...
function AddSkillPanel({ cwd, onInstalled }: { cwd: string; onInstalled: () => void }) {
  ...
  const search = useCallback((q: string) => {
    ...
    setResults([])
    runApi(
      withApi((api) => api.packages.searchSkills({ payload: { query: q.trim() } })),
      {
        onSuccess: ({ results: values }) => {
          const results = values.map((result) => ({ ...result }))
          setResults(results)
  ...
  const install = useCallback(
    (pkg: string) => {
      ...
      const operation =
        scope === "global"
          ? withApi((api) => api.packages.installSkill({ payload: { package: pkg, scope } }))
          : withApi((api) => api.packages.installSkill({ payload: { package: pkg, scope, cwd } }))
      runApi(operation, {
        onSuccess: () => {
...
export function SkillsConfig({ cwd, onClose }: { cwd: string; onClose: () => void }) {
  ...
  const loadSkills = useCallback(() => {
    ...
    setError(null)
    runApi(
      withApi((api) => api.packages.skills({ query: { cwd } })),
      {
