...
export function ChatMinimap({ messages, streamingMessage, scrollContainer, messageRefs }: Props) {
  ...
  useEffect(() => {
    ...
    if (!el) return
    const resizeTargets = el.firstElementChild ? [el, el.firstElementChild] : [el]
    return runBrowser(
  ...
  useEffect(
    () =>
      runBrowser(after("50 millis", updatePositions), {
        onSuccess: () => undefined,
      }),
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      dragCancelRef.current = runBrowser(
        BrowserPlatform.pipe(Effect.flatMap((browser) => browser.onWindowMouseDrag(onMove, onUp))),
        {
          onSuccess: () => {
  ...
  const tooltipPositions = useMemo(() => {
    ...
    const positions = nodes.map((node) => Math.round(node.topRatio * minimapHeightPx - TOOLTIP_HEIGHT / 2))
  ...
  const nearestIndex =
    mouseYRatio !== null && nodes.length > 0
      ? nodes.reduce((best, node) => {
          return Math.abs(node.topRatio - mouseYRatio) < Math.abs(nodes[best].topRatio - mouseYRatio)
            ? node.index
            : best
        }, 0)
      : null
