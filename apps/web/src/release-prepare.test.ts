...
const makeRepository = () => {
  const root = mkdtempSync(join(tmpdir(), "pi-web-release-test-"))
...
it("reuses the release previously produced for the same source sha", () => {
  ...
  try {
    ...
    expect(prepare(fixture.root, sourceSha)).toMatchObject({ mode: "new", version: "0.1.1", sourceSha })
    commitAll(fixture.root, `chore(release): v0.1.1\n\nRelease-Source: ${sourceSha}\n\nRelease-Bump: patch`)
    git(fixture.root, "push", "origin", "main")
    git(fixture.root, "checkout", "--detach", sourceSha)

    expect(prepare(fixture.root, sourceSha)).toMatchObject({ mode: "existing", version: "0.1.1", sourceSha })
    expect(JSON.parse(readFileSync(join(fixture.root, "package.json"), "utf8")).version).toBe("0.1.1")
  } finally {
...
it("rejects a source push that manually changes the CI-owned version", () => {
  ...
  try {
    ...
    git(fixture.root, "push", "origin", "main")
    expect(() => prepare(fixture.root, driftedSource)).toThrow("package version is CI-owned; expected 0.1.1")
  } finally {
    rmSync(fixture.root, { recursive: true, force: true })
    rmSync(fixture.remote, { recursive: true, force: true })
  }
