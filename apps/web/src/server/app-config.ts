...
export class AppConfig extends Context.Service<AppConfig, AppConfigValue>()("pi-web/server/AppConfig") {}

const Home = Config.string("HOME").pipe(Config.orElse(() => Config.string("USERPROFILE")))

const AppConfigEffect = Config.all({
  home: Home,
  platform: Config.string("PI_WEB_PLATFORM").pipe(Config.withDefault(process.platform)),
