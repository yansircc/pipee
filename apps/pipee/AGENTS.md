...
bin/pipee.js                packaged Nitro CLI adapter
```
...
Nitro emits `.output/server/index.mjs` with the `node-server` preset. `bin/pipee.js` parses `-p/--port` and `-H/--hostname`, imports that entry, waits for `/api/health`, and then opens the browser unless disabled.
