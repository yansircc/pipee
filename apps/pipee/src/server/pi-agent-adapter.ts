      ...
      yield* Effect.tryPromise({
        try: () =>
          inner.bindExtensions!({
            ...
            shutdownHandler: () =>
              uiContext.notify("Extension requested shutdown, which is not available in pipee.", "warning"),
            onError: () =>
              publish(
    ...
    Effect.scoped(
      Effect.gen(function* () {
        const directory = yield* fs
          .makeTempDirectoryScoped({ prefix: "pipee-models-validate-" })
  ...
  const testModelConfig = (providerName: string, provider: JsonValue, model: JsonValue) =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const directory = yield* fs
          .makeTempDirectoryScoped({ prefix: "pipee-model-test-" })
  ...
  return PiAgentAdapter.of({
    ...
    exportHtml: (filePath) =>
      Effect.scoped(
        Effect.gen(function* () {
          const directory = yield* fs
            .makeTempDirectoryScoped({ prefix: "pipee-export-" })
