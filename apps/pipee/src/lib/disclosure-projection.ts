...
export type DisplayRow =
  ...
  | {
      ...
      readonly id: string
      readonly turnId?: string
      readonly node: Extract<TurnFlowNode | DocumentNode, { kind: "presentation" }>
    }
  | {
...
const sameRow = (left: DisplayRow, right: DisplayRow): boolean => {
  ...
  if (left.kind === "extension-content" && right.kind === "extension-content")
    return left.node === right.node && left.expanded === right.expanded
  if (left.kind === "presentation" && right.kind === "presentation") return left.node === right.node
...
export function projectDisclosure(
  ...
  const pushTurn = (turn: TurnNode) => {
    ...
    for (const node of turn.flow) {
      if (node.kind === "assistant-content") {
        ...
        rows.push({ kind: "presentation", id: node.id, turnId: turn.id, node })
      } else {
  ...
  for (const node of document.nodes) {
    ...
    else if (node.kind === "presentation") rows.push({ kind: "presentation", id: node.id, node })
