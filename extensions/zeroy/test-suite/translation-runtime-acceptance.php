...
function zeroy_translation_activate_policy_fixture(): void
{
...
$configured = zeroy_runtime_update_site_config([
    ...
    'translationProfile' => [
        ...
        'localeGuidance' => ['en' => 'Use direct international technical English.'],
        'glossary' => [['source' => '环模', 'translations' => ['en' => 'ring die']]],
...
$job = zeroy_localization_translation_job(['kind' => 'post', 'id' => $post_id], 'en');
...
$published = zeroy_localization_publish_translation(['kind' => 'post', 'id' => $post_id], 'en', (int) $draft['revision']);
...
$published_page = wp_remote_get(zeroy_runtime_route_url('en', $route), ['timeout' => 15]);
$published_html = is_wp_error($published_page) ? '' : wp_remote_retrieve_body($published_page);
zeroy_translation_accept(!is_wp_error($published_page) && wp_remote_retrieve_response_code($published_page) === 200 && str_contains($published_html, 'hreflang="en"') && str_contains($published_html, 'rel="canonical"'), 'Published page must emit zeroY canonical and hreflang links.');
$published_coverage = zeroy_translation_inventory_locale($post_id, 'en');
...
$unpublished = zeroy_localization_unpublish_translation(['kind' => 'post', 'id' => $post_id], 'en', (int) $published['revision']);
...
$head = zeroy_localization_overlay_head(['kind' => 'post', 'id' => $post_id], 'en');
...
$republished = zeroy_localization_publish_translation(['kind' => 'post', 'id' => $post_id], 'en', (int) $unpublished['revision']);
...
$shared_changed = zeroy_localization_translation_job(['kind' => 'post', 'id' => $post_id], 'en');
...
$stale = zeroy_localization_translation_job(['kind' => 'post', 'id' => $post_id], 'en');
...
$stale_coverage = zeroy_translation_inventory_locale($post_id, 'en');
zeroy_translation_accept(($stale_coverage['translation']['stale'] ?? 0) === 1, 'Inventory must project only the stale translated field.');
$stale_publish = zeroy_localization_publish_translation(['kind' => 'post', 'id' => $post_id], 'en', (int) $stale['expectedRevision']);
...
$repaired_publish = zeroy_localization_publish_translation(['kind' => 'post', 'id' => $post_id], 'en', (int) $repaired['revision']);
...
$reordered = zeroy_localization_translation_job(['kind' => 'post', 'id' => $post_id], 'en');
$reordered_ids = array_column($reordered['fields'], 'fieldId');
zeroy_translation_accept(!is_wp_error($reordered) && in_array('/acf/field_zeroy_specs/motor/field_zeroy_spec_label', $reordered_ids, true) && in_array('/acf/field_zeroy_specs/die/field_zeroy_spec_label', $reordered_ids, true) && !in_array('stale', array_map(static fn(array $field): string => $field['status'], $reordered['fields']), true), 'Repeater reorder must preserve itemKey translation identity.');
update_field('field_zeroy_specs', [['code' => 'motor', 'label' => '主电机'], ['code' => 'motor', 'label' => '备用电机']], $post_id);
$duplicate = zeroy_localization_translation_job(['kind' => 'post', 'id' => $post_id], 'en');
...
$media_job = zeroy_localization_translation_job(['kind' => 'media', 'id' => $attachment_id], 'en');
...
$media_publish = zeroy_localization_publish_translation(['kind' => 'media', 'id' => $attachment_id], 'en', (int) $media_draft['revision']);
$media = zeroy_locale_media($attachment_id, 'en');
...
$term_job = zeroy_localization_translation_job(['kind' => 'term', 'taxonomy' => 'category', 'id' => $term_id], 'en');
...
$term_publish = is_wp_error($term_draft) ? $term_draft : zeroy_localization_publish_translation(['kind' => 'term', 'taxonomy' => 'category', 'id' => $term_id], 'en', (int) $term_draft['revision']);
$term_content = zeroy_localization_term_content('category', $term_id, 'en');
...
$menu_job = zeroy_localization_translation_job(['kind' => 'menu', 'id' => $menu_id], 'en');
...
$menu_publish = is_wp_error($menu_draft) ? $menu_draft : zeroy_localization_publish_translation(['kind' => 'menu', 'id' => $menu_id], 'en', (int) $menu_draft['revision']);
$menu = zeroy_locale_menu($menu_id, 'en');
...
zeroy_translation_accept(!is_wp_error($menu_publish) && ($menu_post['url'] ?? null) === zeroy_runtime_route_url('en', $route) && ($menu_term['url'] ?? null) === zeroy_runtime_route_url('en', 'category/' . get_term_field('slug', $term_id, 'category')) && ($menu_custom['url'] ?? null) === 'https://example.com/', 'Menu labels and target URLs must resolve at the current locale.');

$site_copy_job = zeroy_localization_translation_job(['kind' => 'site-copy', 'id' => 'default'], 'en');
...
$site_copy_publish = is_wp_error($site_copy_draft) ? $site_copy_draft : zeroy_localization_publish_translation(['kind' => 'site-copy', 'id' => 'default'], 'en', (int) $site_copy_draft['revision']);
$site_copy = zeroy_localization_site_copy('en');
...
echo wp_json_encode([
    ...
    'postId' => $post_id,
    'url' => zeroy_runtime_route_url('en', $route),
]);
