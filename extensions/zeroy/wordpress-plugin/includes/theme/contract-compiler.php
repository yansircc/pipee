...
function zeroy_runtime_compile_theme_contract(string $theme_artifact_id, string $site_logic_artifact_id): array|WP_Error
{
    ...
    $schema = zeroy_runtime_schema_diagnostics_from_path(zeroy_runtime_artifact_directory($theme_artifact_id) . '/zeroy.schema.json', zeroy_runtime_artifact_directory($theme_artifact_id));
    ...
    $theme_manifest = zeroy_runtime_theme_runtime_manifest(zeroy_runtime_artifact_directory($theme_artifact_id));
    if (is_wp_error($theme_manifest)) return $theme_manifest;
    $site_logic_contract = zeroy_runtime_decode_json((string) $logic['contract_json']);
    $site_logic_contract = is_wp_error($site_logic_contract) ? $site_logic_contract : zeroy_runtime_normalize_site_logic_contract($site_logic_contract);
    ...
    $templates = [[
        ...
        'kind' => 'front-page',
        'inputSchema' => ['type' => 'object', 'properties' => ['locale' => ['type' => 'string']], 'required' => ['locale']],
        'requiredCapabilities' => $requirements,
    ]];
    foreach ($content_schemas as $content_schema) {
        $templates[] = [
            'templateId' => 'singular:' . $content_schema['schemaId'],
            'kind' => 'singular',
            'inputSchema' => ['type' => 'object', 'properties' => ['objectId' => ['type' => 'integer'], 'locale' => ['type' => 'string']], 'required' => ['objectId', 'locale']],
            'requiredCapabilities' => $requirements,
        ];
    }
    foreach ($collections as $collection) {
        $templates[] = [
            ...
            'inputSchema' => ['type' => 'object', 'properties' => ['locale' => ['type' => 'string'], 'page' => ['type' => 'integer']], 'required' => ['locale']],
            'requiredCapabilities' => $requirements,
        ];
    }
    $templates[] = ['templateId' => 'search', 'kind' => 'search', 'inputSchema' => ['type' => 'object', 'properties' => ['locale' => ['type' => 'string'], 'query' => ['type' => 'string']], 'required' => ['locale', 'query']], 'requiredCapabilities' => $requirements];
    $templates[] = ['templateId' => '404', 'kind' => '404', 'inputSchema' => ['type' => 'object', 'properties' => ['locale' => ['type' => 'string']], 'required' => ['locale']], 'requiredCapabilities' => $requirements];
    $contract = [
