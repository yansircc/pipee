...
function zeroy_checkout_reachability(): array
{
    ...
    foreach ($wpdb->get_results('SELECT release_id, commit_hash, proof_id, state FROM ' . zeroy_runtime_table('site_releases'), ARRAY_A) ?: [] as $release) {
        ...
        if (is_string($release['proof_id'] ?? null) && $release['proof_id'] !== '') {
            ...
            if ($proof === null || !hash_equals((string) ($proof['commit_hash'] ?? ''), (string) ($release['commit_hash'] ?? ''))) $issues[] = ['code' => 'site_release_proof_commit_mismatch', 'releaseId' => $release['release_id'], 'proofId' => $release['proof_id']];
    ...
    return ['roots' => array_keys($roots), 'commits' => array_keys($commits), 'objects' => array_keys($objects), 'issues' => $issues];
}
...
function zeroy_checkout_gc(int $grace_seconds = 604800): array|WP_Error
{
    ...
    return ['deletedCommits' => $deleted_commits, 'deletedObjects' => $deleted_objects, 'reachableCommits' => count($commit_set), 'reachableObjects' => count($object_set)];
}
