...
if (candidate.releasable) {
  ...
  assertReleaseRecordCommit({
    record,
    parents: run("git", ["show", "-s", "--format=%P", head], { capture: true }).trim().split(/\s+/),
    manifestVersions: Object.fromEntries(
      pipeeConfig().packages.map(({ id, path }) => [
        id,
    ...
    sourceManifestVersions: Object.fromEntries(
      pipeeConfig().packages.map(({ id, path }) => [
        id,
        JSON.parse(run("git", ["show", `${record.source}:${path}/package.json`], { capture: true }))
          .version,
      ]),
    ),
    packageIds: pipeeConfig().packages.map(({ id }) => id),
    packageManifestPaths: Object.fromEntries(
      pipeeConfig().packages.map(({ id, path }) => [id, `${path}/package.json`]),
    ),
...
for (const entry of pipeeConfig().packages.filter(({ id }) => projectedIds.includes(id))) {
  ...
  const stagingDirectory = mkdtempSync(join(tmpdir(), "pipee-verify-"));
  try {
    ...
    while (pending.length > 0) {
      ...
      for (const name of readdirSync(directory)) {
        ...
        if (statSync(path).isDirectory()) pending.push(path);
          ...
          assertPipeeBrand(
            source,
            `${entry.id} archive file ${path.slice(packageDirectory.length + 1)}`,
          );
