...
export const readDistributionContract = (root, config) => {
  ...
  const webManifest = manifest.pipee?.web;
  if (webManifest !== undefined) {
    assert.equal(webManifest.contract, "pipee/web-surface@2", "unknown Pipee web surface contract");
    assert.equal(typeof webManifest.title, "string", "Pipee web surface title must be a string");
    assert.ok(webManifest.title.trim(), "Pipee web surface title must not be empty");
    assert.equal(
      config.profile.kind,
      "multi-file",
      "Pipee web surfaces require the multi-file profile",
    );
    assert.ok(assets.includes("dist/web"), "Pipee web surfaces require dist/web in profile.assets");
    const document = normalizeRelative(webManifest.document, "Pipee web surface document");
    assert.ok(
      document.startsWith("dist/web/"),
      "Pipee web surface document must be under dist/web/",
    );
    assert.ok(
      existsSync(resolve(root, document)),
      `missing Pipee web surface document: ${document}`,
    );
  }
